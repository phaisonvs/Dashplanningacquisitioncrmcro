...
const orderEvidenceTags = (tags: ConversionExperienceItem['tags']) =>
  [...tags]
    ...
    .sort((left, right) => EVIDENCE_TAG_ORDER.indexOf(left) - EVIDENCE_TAG_ORDER.indexOf(right));
...
const ObjectiveBlock = ({ text, compact = false }: { text: string; compact?: boolean }) => (
  ...
  >
    ...
    <div style={{ color: 'rgba(255,255,255,0.68)', fontSize: 'var(--paragrafo)', lineHeight: 1.5, fontWeight: 300 }}>
      {text}
    </div>
  </div>
);
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: isCompact ? '16px' : '20px', padding: isCompact ? '20px' : '26px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '14px', flexDirection: 'row', flexWrap: 'nowrap' }}>
          <div data-ui="card-evidencia-conteudo" style={{ display: 'flex', minWidth: 0, flex: '1 1 auto', alignItems: 'flex-start', gap: isCompact ? '10px' : '12px', flexDirection: 'column' }}>
            <div data-ui="card-evidencia-titulo" style={{ color: WHITE, fontSize: 'var(--paragrafo-grande)', fontWeight: 700, marginBottom: '0', lineHeight: 1.25 }}>
              {item.title}
            </div>
            <div data-ui="card-evidencia-tags" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <div data-ui="card-evidencia-status" style={{ alignSelf: 'flex-start' }}>
...
export function ConversionExperienceSection({
  ...
  return (
    ...
    >
      ...
      <div data-ui="grid-evidencias-conversao" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isCompact ? 'repeat(2, minmax(0, 1fr))' : 'repeat(3, minmax(0, 1fr))', gap: isCompact ? '16px' : '22px' }}>
        {items.map((item) => (
